    ...
    try:
        ...
        for col in ['Start', 'Finish']:
            ...
            df[col] = pd.to_datetime(
                f"{DUMMY_DATE} " + temp_dt.dt.strftime('%H:%M:%S'),
                errors='coerce'
            )
...
def save_data(full_df, file_path="tasks.csv"):
    """
    保存数据到CSV。
...
if df_original is not None:
    ...
    if search_term:
        df_filtered = df_filtered[df_filtered['Task'].str.contains(search_term, case=False, na=False)]
    ...
    with col_editor:
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "Bot": st.column_config.SelectboxColumn("机器人", options=all_bots, required=True)
            },
    ...
    if not plot_df.empty:
        # 计算持续时间
        plot_df['Duration_seconds'] = (plot_df['Finish'] - plot_df['Start']).dt.total_seconds()
        plot_df['Duration'] = (plot_df['Duration_seconds'] / 60).round().astype(int).astype(str) + " min"
        ...
        fig.update_yaxes(
            ...
            categoryorder='array', 
            categoryarray=plot_df['Task'].unique()[::-1] # 反转以配合 Plotly 从下往上的绘制顺序
        )
